...
uint64_t evalMultiplicationChain(std::string::const_iterator &from, std::string::const_iterator &to);
...
int main() {
    ...
    for (const std::string &line: lines) {
        ...
        sum += evalMultiplicationChain(from, to);
    }
...
uint64_t evalParentheses(std::string::const_iterator &from, std::string::const_iterator &to) {
    ...
    ++from;
    uint64_t result = evalMultiplicationChain(from, to);
    if(from == to || *from != ')') {
        throw std::runtime_error("Expected a (");
    }
    ++from;
...
uint64_t evalSumChain(std::string::const_iterator &from, std::string::const_iterator &to) {
...
uint64_t evalMultiplicationChain(std::string::const_iterator &from, std::string::const_iterator &to) {
    uint64_t result = evalSumChain(from, to);
    while(from != to) {
        auto c = *from;
        if (c == '*') {
            ++from;
            result *= evalSumChain(from, to);
        } else if(c == ')') {
            break;
        } else {
            throw std::runtime_error(std::string("Unexpected char when parsing number: ") + c);
