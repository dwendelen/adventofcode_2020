...
int main() {
    ...
    for (int y = 0; y < lines.size(); ++y) {
        ...
        for (int x = 0; x < line.size(); ++x) {
            if (line[x] == '#') {
                current[Coordinate(x, y, 0, 0)] = true;
            }
        }
    }
            ...
            for (int64_t xx = element.x - 1; xx <= element.x + 1; ++xx) {
                for (int64_t yy = element.y - 1; yy <= element.y + 1; ++yy) {
                    for (int64_t zz = element.z - 1; zz <= element.z + 1; ++zz) {
                        for (int64_t ww = element.w - 1; ww <= element.w + 1; ++ww) {
                            const Coordinate &neighbour = Coordinate(xx, yy, zz, ww);
                            ...
                            for (int64_t xxx = xx - 1; xxx <= xx + 1; ++xxx) {
                                for (int64_t yyy = yy - 1; yyy <= yy + 1; ++yyy) {
                                    for (int64_t zzz = zz - 1; zzz <= zz + 1; ++zzz) {
                                        for (int64_t www = ww - 1; www <= ww + 1; ++www) {
                                            if (xxx == xx && yyy == yy && zzz == zz && www == ww) {
                                                continue;
                                            }
                                            count += current.count(Coordinate(xxx, yyy, zzz, www));
                                        }
                                    }
                                }
                            }
